...
export class KalemComponent implements OnInit {
  ...
  getKalemler(islemInternalNo) {
    if (
      ...
      this._kalemler= this.session.Kalemler
      console.log( this._kalemler);
        if ( this._kalemler == null) {
          this.openSnackBar(this.session.islemInternalNo + " ait Kalem Bulunamadı", "Tamam");
          return;
        }
 
        this.kalemForm.setValue({
          beyanInternalNo: this._kalemler[0].beyanInternalNo,
          gtip: this._kalemler[0].gtip,
          kalemSiraNo: this._kalemler[0].kalemSiraNo,
